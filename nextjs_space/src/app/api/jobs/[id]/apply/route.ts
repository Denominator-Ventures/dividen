...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (job.posterId === user.id) return NextResponse.json({ error: 'Cannot apply to your own job' }, { status: 400 });

  // Check for existing application
  const existing = await prisma.jobApplication.findUnique({
    where: { jobId_applicantId: { jobId: params.id, applicantId: user.id } },
  });
