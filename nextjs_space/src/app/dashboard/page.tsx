...
export default function DashboardPage() {
  ...
  // Handle onboarding Phase 1 completion (API key submitted) — transition to chat-based phases
  const handleOnboardingComplete = useCallback(async (options?: { triggerDiviIntro?: boolean }) => {
    setShowOnboarding(false);
    setActiveTab('chat');

    if (options?.triggerDiviIntro) {
      // Send a hidden system message as the "trigger" (not visible to user)
      try {
        await fetch('/api/chat/send', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            message: '[SYSTEM: User just completed API key setup. Starting phased onboarding. Do not respond — onboarding messages are generated by the system.]',
          }),
        });
      } catch {}

      // Advance to phase 1 (divi-settings) and generate the interactive message
      try {
    ...
    fetch('/api/settings')
      ...
      .then((d) => {
        if (d.success) {
          ...
          } else if (!d.data.user.hasCompletedOnboarding && (!d.data.user.onboardingPhase || d.data.user.onboardingPhase === 0)) {
            // Phase 0: show the API key wizard
            setTimeout(() => setShowOnboarding(true), 400);
          } else if (d.data.user.onboardingPhase > 0 && d.data.user.onboardingPhase < 6) {
            // Phases 1-5: resume chat-based onboarding
            setTimeout(() => resumeOnboarding(d.data.user.onboardingPhase), 500);
          }
